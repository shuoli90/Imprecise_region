...
def Imprecise_region(plausible_distribution, delta):
  """Construct imprecise region from plausible distributions.
    ...
    """
  A = []
  upper_probabilities = plausible_distribution[:, 0]
  lower_probabilities = plausible_distribution[:, 1]
  sorted_class = jnp.argsort(lower_probabilities)[::-1]
  n_classes = plausible_distribution.shape[0]
  for i in range(n_classes):
    A.append(sorted_class[i].item())
    lower_sum = jnp.sum(lower_probabilities[A]).item()
    upper_sum = jnp.sum(upper_probabilities).item() - jnp.sum(upper_probabilities[A]).item()
    P_A = max(lower_sum, 1-upper_sum)
    if P_A >= 1 - delta:
      return A
  return A